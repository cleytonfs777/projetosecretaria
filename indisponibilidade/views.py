...
def registrar_indisp(request):
    ...
    if request.method == 'POST':
        ...
        if not datainitindisp and not datahorainitindisp:
            messages.add_message(request, messages.ERROR, 'Nenhum campo de data deve estar em branco!')
            return redirect(reverse('registrar_indisp'))

        if datainitindisp:

            datainit = datetime.strptime(datainitindisp + " 00:00", '%d/%m/%Y %H:%M')

            if datafinalindisp:
                datafinal = datetime.strptime(datafinalindisp + " 23:59", '%d/%m/%Y %H:%M')
            else:
                datafinal = None  # ou algum valor padrão, caso necessário

        else:
            if datahorainitindisp:
                datainit = datetime.strptime(datahorainitindisp, '%d/%m/%Y %H:%M')
            else:
                datainit = None  # ou algum valor padrão, caso necessário

            if datahorafinalindisp:
                datafinal = datetime.strptime(datahorafinalindisp, '%d/%m/%Y %H:%M')
            else:
                datafinal = None  # ou algum valor padrão, caso necessário

        # Verifica se algum dos campo estão em branco
        if not typeindisp or not datainit or not datafinal or not militar_id:
            messages.add_message(request, messages.ERROR, 'Nenhum campo deve estar em branco!')
            return redirect(reverse('registrar_indisp'))
        ...
        if indisps:
            for indisp in indisps:
                ...
                if indisp_ini <= datainit <= indisp_end or indisp_ini <= datafinal <= indisp_end:
                    messages.add_message(request, messages.ERROR, 'Já existe uma indisponibilidade para o militar neste período!')
                    return redirect(reverse('registrar_indisp'))
        ...
        if datainit > datafinal:
            messages.add_message(request, messages.ERROR, 'A data inicial não pode ser maior que a data final!')
            return redirect(reverse('registrar_indisp'))
        ...
        messages.add_message(request, messages.SUCCESS, 'Indisponibilidade cadastrada com sucesso!')
        return redirect(reverse('registrar_indisp'))
...
def delete_indisp(request, id_indisp):
    if request.method == 'POST':
        ...
        try:
            ...
            messages.add_message(request, messages.SUCCESS, 'Indisponibilidade deletada com sucesso!')
            return redirect(reverse('listar_indisp'))
        except IndispMilitar.DoesNotExist:
            messages.add_message(request, messages.ERROR, 'Indisponibilidade não existe!')
            return redirect(reverse('listar_indisp'))
    else:
        return redirect(reverse('listar_indisp'))
...
def edit_indisp(request, id_indisp):
    if request.method == 'POST':
        ...
        indispmilitar.save()
        messages.add_message(request, messages.SUCCESS, 'Indisponibilidade atualizada com sucesso!')
        return redirect(reverse('listar_indisp'))
    else:
        return redirect(reverse('listar_indisp'))
...
def registrar_ferlib(request):
    if request.method == 'POST':
        ...
        if not only_date and (not horainicial or not horafinal):
            messages.add_message(request, messages.ERROR, 'Informe a Hora inicial / final ou marque a opção "Especificar Apenas Data"')
        ...
        if nomeferlib == '' or  classferlib == '' or dataferlib == '':
            messages.add_message(request, messages.ERROR, 'Campos obrigatórios não podem ficar em branco: Nome, Classe e Data')
            return redirect(reverse('registrar_ferlib'))     
        ...
        if horainicial and horafinal:
            if horainicial > horafinal:
                messages.add_message(request, messages.ERROR, 'A hora inicial não pode ser maior que a hora final!')
                return redirect(reverse('registrar_ferlib'))
        ...
        if ferlib:
            messages.add_message(request, messages.ERROR, 'Já existe um feriado ou liberação com o mesmo nome e data!')
            return redirect(reverse('registrar_ferlib'))
        ...
        diazero.save()
        messages.add_message(request, messages.SUCCESS, 'Feriado ou Liberação cadastrado com sucesso!')
        return redirect(reverse('registrar_ferlib'))
...
def delete_ferlib(request, id_ferlib):
    if request.method == 'POST':
        ...
        try:
            ...
            diazero.delete()
            messages.add_message(request, messages.SUCCESS, 'Feriado ou Liberação deletado com sucesso!')
            return redirect(reverse('listar_ferlib'))
        except DiaZero.DoesNotExist:
            messages.add_message(request, messages.ERROR, 'Feriado ou Liberação não existe!')
            return redirect(reverse('listar_ferlib'))
